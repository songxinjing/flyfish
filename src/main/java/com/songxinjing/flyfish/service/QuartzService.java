...
public class QuartzService {
	...
	public void uploadImg() {
		...
		String hql = "from Goods where isUpload = :isUpload";
		Map<String, Object> paraMap = new HashMap<String, Object>();
		paraMap.put("isUpload", 0);
		List<Goods> list = goodsService.findPage(hql, 0, 300, paraMap);
		// 首先全部标志为正在上传，避免其他任务重复上传
		for (Goods g : list) {
			g.setIsUpload(2); // 2表示正在上传
		...
		int fail = 0;
		for (Goods g : list) {
			GoodsImg goodsImg = goodsImgService.find(g.getSku());
			if (goodsImg == null) {
				goodsImg = new GoodsImg();
				goodsImg.setSku(g.getSku());
			...
			for (String key : ExcelTemp.WISH_FIELD.keySet()) {
				...
				if (StringUtils.isNotEmpty(name) && name.contains("Img")) {
					String gpUrl = (String) ReflectionUtil.getFieldValue(g, name);
					...
					if (StringUtils.isNotEmpty(gpUrl) && StringUtils.isEmpty(imgName)) {
						...
						String tempSku = g.getSku();
						if (name.equals("mainImgUrl")) {
							Goods goods = goodsService.find(g.getSku());
			...
			if (gpSuccess) {
				g.setIsUpload(1); // 1表示成功
				goodsService.update(g);
			} else {
				logger.info("SKU " + g.getSku() + " 图片上传存在失败情况");
				fail++;
			}
			all--;
			logger.info("任务(" + hour + ") SKU " + g.getSku() + " 图片上传完毕(剩余：" + all + ")");
		}
